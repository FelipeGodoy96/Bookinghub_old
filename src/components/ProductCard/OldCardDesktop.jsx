...
export const OldCardDesktop = ({ product }) => {
  ...
  return (
    <div className="product-card bg-white rounded-b-lg drop-shadow-md shadow-lg md:w-full md:min-h-[280px] overflow-hidden">
      <div className="c-image flex md:flex-row w-full h-full">
      <img src={product.source} className="md:w-1/2 md:max-w-1/2 sm:w-1/3 rounded-b-md" />
      <div className="c-content flex flex-col pt-4 pb-2 px-4 w-full">
        <div className="c-content__header">
          <article className="title flex justify-between h-8 text-center">
            <p className="text-md text-slate-400 flex gap-2 self-center">
              ...
              <span className="flex gap-1">{stars.map((star) => <Star key={star}/>)}</span>
            </p>
        ...
        <div className="c-content__body--description my-2" style={{display: expandedText ? 'flex' : 'block', flexDirection: expandedText ? 'row' : 'column' }}>
          <p>
            {expandedText
              ? product.descricao + " "
